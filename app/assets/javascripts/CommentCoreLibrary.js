...
/**=================================重写类开始==============================================**/
var OldScrollComment = ScrollComment;
var ScrollComment = (function(_super){
  __extends(ScrollComment, _super)
  function ScrollComment(parent, data){
    _super.call(this, parent, data);
...
var OldCoreComment = CoreComment;
var CoreComment = (function(_super){
  __extends(CoreComment, _super);
  function CoreComment(parent, data){
    _super.call(this, parent, data);
    this.html = data['html'] || '';
  ...
  CoreComment.prototype.init = function(recycle){
    _super.prototype.init.call(this, recycle);
    this.dom.attributes.id = this.id;
    $(this.dom).append(this.html);
  }
  return CoreComment;
})(OldCoreComment);

//var OldCommentSpaceAllocator = CommentSpaceAllocator;
var MyCommentSpaceAllocator = (function (_super) {
  __extends(MyCommentSpaceAllocator, _super);
  function MyCommentSpaceAllocator(width, height) {
    _super.call(this, width, height);
  }

  MyCommentSpaceAllocator.prototype.assign = function (comment, cindex) {
    var y = this._height / 5, offset = this._height / 5;
    while (this._pools.length <= cindex) {
      this._pools.push([]);
    }
    var pool = this._pools[cindex];
  ...
  return MyCommentSpaceAllocator;
})(CommentSpaceAllocator);
/**=================================重写类结束==============================================**/
...
function CommentFilter(){
  ...
  this.doModify = function(cmt){
    for(var k=0;k<this.modifiers.length;k++){
      cmt = this.modifiers[k](cmt);       // this.modifiers[k]是什么Object???
    }
    return cmt;
  };
...
var CommentManager = (function() {
  ...
  function CommentManager(stageObject){
    ...
    this.csa = {
      ...
      myscroll: new MyCommentSpaceAllocator(0,0)
    };
  ...
  CommentManager.prototype.send = function(data){
    ...
    if(data.mode === 1 || data.mode === 2 || data.mode === 6 || data.mode === 101 || data.mode === 102){
    ...
    switch(cmt.mode){
      ...
      case 101:{this.csa.myscroll.add(cmt);}break;
    }
    cmt.y = cmt.y;
